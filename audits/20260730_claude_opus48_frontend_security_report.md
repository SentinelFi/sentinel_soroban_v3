...
## Resolution Status (2026-07-30, branch `frontend_audit_fixes`)

Triage rule applied: fix anything exploitable/observable in the deployed
production app; for issues that only manifest in local development or are
pure ops/key-hygiene actions, record the disposition instead of changing
code. Every code fix was tested and checked for production-deploy safety.

| ID | Severity | Status |
| --- | --- | --- |
| FSA-H01 | High | **Fixed.** RLS enabled at self-create in the four app paths + conditional hardening migration; proven live (anon read/insert denied, owner unaffected). e2e 26/26 + 25/25, tsc clean. |
| FSA-M01 | Medium | **Fixed (clickjacking scope).** `frame-ancestors 'none'`, `X-Frame-Options: DENY`, `nosniff`, `Referrer-Policy`, conservative HSTS added to `vercel.json` + `vercel.backend.json`. A full `script-src`/`connect-src` CSP is deliberately deferred: its origins are env-specific (RPC/Horizon/Supabase differ per deployment) and it can't be validated without a staging deploy, so shipping it blind would risk breaking prod. Follow-up: validate a full CSP on a preview deploy. |
| FSA-M02 | Medium | **Fixed.** `verifyAdmin` now requires `email_confirmed_at`; gate extracted to `resolveAdminEmail` and unit-tested 8/8. Non-breaking — magic-link/OAuth (the flows in use) already set it. |
| FSA-L01 | Low | **Fixed.** Non-blocking wallet↔app network-mismatch banner (`WalletProvider` flag + `App` banner). Never hard-blocks signing and only trips on a positively-known differing network, so it cannot lock out a working app. |
| FSA-L02 | Low | **Fixed.** `allowHttp` gated to `stellarNetwork === "LOCAL"` in all six contract clients; the production (TESTNET) bundle now builds `allowHttp: false`. Durable — these are hand-maintained wrappers, not regenerated by `rebuild-bindings.sh`. |
| FSA-L03 | Low | **No code change — not a production security issue.** Trusting the non-Freighter address from localStorage is same-origin and display-only (a poisoned value cannot move funds; the wallet still auth-signs as the real key). Left as-is to avoid destabilizing the wallet reconnect flow. |
| FSA-L04 | Low | **Ops action (no deployed-code issue).** Secrets live only in the untracked local `.env`, never in git history or the bundle (verified). Requires key rotation by the operator if the file was ever shared; not fixable in code. |
| FSA-I01 | Info | **Accepted / mitigated.** Admin JWT in localStorage is exploitable only via an XSS sink, and none exist; server-side `ADMIN_EMAILS` re-check backs it. `persistSession: false` would regress admin UX for negligible gain. |
| FSA-I02 | Info | **Ops verify.** The committed anon key is public-by-design (already in the bundle). Confirm Supabase Auth rate limits + a non-wildcard redirect-URL allowlist to bound OTP-spam abuse. |

Related (tracked in the off-chain report): the cron-auth fail-open
(**OCA-L02**) was also fixed on this branch — `CRON_SECRET` is now
required and the handler fails closed.
