...
1. **Route whitelisting** — a human fills [dapp/config/routes.testnet.json](dapp/config/routes.testnet.json) and runs `npm run whitelist:routes`, which calls `GovernanceModule.whitelist_route` as a governance admin. Terms (premium, payoff, delay-hours threshold) fold with global defaults; routes can later be disabled, re-enabled, updated, or removed. Only routes reporting `Active` status are buyable. Listing new routes is always human-initiated.
