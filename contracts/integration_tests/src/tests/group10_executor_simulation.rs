...
fn full_executor_cycle_settles_all_flights_in_one_pass() {
    ...
    // in the same orchestration cycle. Mature it so the maintenance tick
    // below may price it.
...
// =========================================================================
// Targeted fast path — outcome writer drives classify_flight + settle_flight
// =========================================================================

#[test]
fn targeted_classify_and_settle_bypasses_sweep_rotation() {
    // Simulates the executor's post-outcome-write behaviour: whichever job
    // writes set_landed / set_cancelled immediately calls the exact-tuple
    // classify_flight + settle_flight, so the settlement barrier releases
    // without waiting for either rotating sweep to reach the flight — even
    // while unrelated flights occupy the active set.
    let t = TestEnv::new();
    whitelist_extra_routes(&t);

    let on_time_buyer = Address::generate(&t.env);
    let delayed_buyer = Address::generate(&t.env);
    t.buy(&on_time_buyer); // AA100
    buy_on(
        &t,
        &delayed_buyer,
        &flight_delayed(),
    ...
    // Fetcher writes the delayed flight's outcome; AA100 stays unresolved in
    // the same active set.
    t.oracle.set_estimated_arrival(
        &t.oracle_account,
        &flight_delayed(),
    ...
    // Barrier engaged: the written outcome blocks LP entry until settled.
    assert!(t.oracle.has_pending_outcomes());

    // Executor drives the exact tuple straight through both stages.
    assert!(t
        .ctrl
        .classify_flight(&t.keeper, &flight_delayed(), &FLIGHT_DATE));
    assert!(t
        .ctrl
        .settle_flight(&t.keeper, &flight_delayed(), &FLIGHT_DATE));

    // The targeted flight is fully settled and the barrier is released...
    assert_eq!(
        t.oracle
            .get_flight_data(&flight_delayed(), &FLIGHT_DATE)
            .status,
    ...
    // ...while the unrelated flight was never touched.
    assert_eq!(
        t.oracle
            .get_flight_data(&symbol_short!("AA100"), &FLIGHT_DATE)
            .status,
        oracle_aggregator::FlightStatus::NotInitiated
    );

    // LP entry prices again immediately, and the traveler can claim.
