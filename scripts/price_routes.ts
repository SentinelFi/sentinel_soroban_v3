 ...
 *   premium     = clamp(p_covered × payoff × 1.3, rails $10–$30)
...
import { expectedLossPremiumUnits, clampPremium } from "../dapp/api/_lib/route_rules";
...
async function main(): Promise<void> {
  ...
  const out = {
    $schema_note:
      "STAGED route whitelist — generated by scripts/price_routes.ts, reviewed by the admin, seeded on-chain by scripts/seed_routes.ts. Regenerating overwrites; nothing here is live until seeded.",
    generated_at: new Date().toISOString(),
    priced_for_date: forDate.toISOString().slice(0, 10),
    model_version: modelVersion,
    payoff_usdc: PAYOFF_USDC,
    delay_hours: DELAY_HOURS,
    rails: { premium_min_usdc: baseUnitsToUsdc(rails.premiumMin), premium_max_usdc: baseUnitsToUsdc(rails.premiumMax) },
    routes,
  };
  ...
  console.log("[price] premium distribution:");
