...
const NAV = [
	{ to: "/", label: "MARKETS" },
	{ to: "/bets", label: "MY POLICIES" },
	{ to: "/house", label: "EARN YIELD" },
]
...
export function TopBar() {
	...
	return (
		<header className="sticky top-0 z-20 border-b-2 border-line bg-page">
				...
				<NavLink to="/" className="flex items-center gap-3">
					...
					<span className="flex flex-col">
						<span className="font-display text-[13px] leading-none text-ink">
							FLIGHTS<span className="text-gold">.FUN</span>
						</span>
						<span className="mt-1 font-body text-[10px] font-semibold tracking-[0.14em] text-sky">
							PARAMETRIC FLIGHT INSURANCE.
							<br />
							PLAYED LIKE A MARKET.
						</span>
					</span>
				</NavLink>
				...
				<nav className="ml-6 hidden items-center gap-1 md:flex">
					{NAV.map((item) => (
						<NavLink
							...
							className={({ isActive }) =>
								cn(
									"px-3 py-2 font-display text-[10px] tracking-[0.04em]",
									isActive
										? "bg-gold text-page"
			...
			<nav className="flex items-center justify-center gap-1 border-t-2 border-line px-4 py-2 md:hidden">
				{NAV.map((item) => (
					<NavLink
						...
						className={({ isActive }) =>
							cn(
								"whitespace-nowrap px-3 py-1.5 font-display text-[10px]",
								isActive
									? "bg-gold text-page"
