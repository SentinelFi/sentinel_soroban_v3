...
export const JOB_REGISTRY: JobInfo[] = [
  ...
  { job: "classifier", path: "/api/cron/classify", schedule: "33 * * * *", intervalMinutes: 60, signer: "keeper", manualRunnable: true, description: "Classify flights for settlement" },
  ...
  { job: "gov_exposure", path: "/api/cron/gov-exposure", schedule: "7 * * * *", intervalMinutes: 60, signer: "gov-admin", manualRunnable: true, description: "Exposure brake — on-chain liability concentration (route + airport) vs vault capacity; ≥50% → `exposure` intervention (pause, capped per run), ≥25% advisory. Also ingests the policies event mirror and appends the exposure_history series (Trends board)" },
