...
export const JOB_REGISTRY: JobInfo[] = [
  ...
  { job: "revive", path: "/api/cron/revive", schedule: "0 6 * * *", intervalMinutes: 1440, signer: "gov-admin", manualRunnable: true, description: "Revive check — re-sweeps the 20 most recently guard-paused routes (route_health); verifiably back in schedule → re-enabled" },
  { job: "gov_exposure", path: "/api/cron/gov-exposure", schedule: "7 * * * *", intervalMinutes: 60, signer: "none", manualRunnable: true, description: "Exposure collector — on-chain liability concentration (route + airport) vs vault capacity → exposure signals (facts only, no chain writes)" },
  { job: "gov_reconcile", path: "/api/cron/gov-reconcile", schedule: "10 * * * *", intervalMinutes: 60, signer: "gov-admin", manualRunnable: true, description: "Governance reconciler — exposure signals → on-chain pauses" },
